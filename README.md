...
#### `fetch_page`
抓取任意网页的正文内容（去除导航栏、脚本等无关元素），最多返回 3000 字符。
...
#### `fetch_prts_wiki`
从 [prts.wiki](https://prts.wiki) 抓取明日方舟角色页面，最多返回 5000 字符。
...
> 数据源：[HLTV](https://www.hltv.org)（网页抓取）、[Liquipedia](https://liquipedia.net/counterstrike)（API）、[5EPlay](https://www.5eplay.com)（API）
> 推荐安装 `cloudscraper` 以绕过 HLTV 的 Cloudflare 防护，不装也能用但可能被拦截。5EPlay 走官方 JSON API，无需 `cloudscraper`。
...
> **注意：** HLTV 的 `live` 模式依赖 JS 动态渲染，HTML 抓取不可靠。获取实时比赛请用 `csgo_matches(match_type="live", source="5eplay")`。
...
获取比赛详情，包含总比分、地图选择（veto）、每张地图比分（含半场）及双方选手完整统计数据（Rating 3.0、K-D、ADR、KAST、HS% 等）。支持 HLTV 和 5EPlay 两个数据源。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `match_id` | str | *必填* | 比赛 ID，HLTV 格式如 `2394896/legacy-vs-tyloo-...`（从 `csgo_matches` 返回链接提取），5EPlay 格式如 `csgo_mc_2394896` 或纯数字 `2394896` |
| `source` | str | `"hltv"` | 数据源：`hltv`（默认）、`5eplay` |

示例：
```
csgo_match_detail("2394896/legacy-vs-tyloo-iem-cologne-major-2026-stage-2")
→ 返回 Overall / Ancient / Overpass 三张表的双方 10 名选手 stats
csgo_match_detail("2394896", source="5eplay")
→ 从 5EPlay 获取同一比赛的详情
```
...
| type | 功能 | params 示例 |
|------|------|-------------|
| `StartUp` | 开始唤醒 | `{"client_type":"Official","start_game_enabled":true}` |
| `Fight` | 刷理智 | `{"stage":"1-7","times":99,"medicine":0}`（`stage` 留空为当前关） |
| `Recruit` | 自动公招 | `{"refresh":true,"select":[4,5],"confirm":[3,4,5]}` |
| `Infrast` | 基建换班 | `{"facility":["Mfg","Trade","Power","Control","Reception","Office","Dorm"]}` |
| `Mall` | 信用购物 | `{"shopping":true}` |
| `Award` | 领取奖励 | — |
| `Roguelike` | 肉鸽 | `{"theme":"Phantom"/"Mizuki"/"Sami"/"Sarkaz"}` |
| `Copilot` | 自动作业 | `{"filename":"作业JSON路径"}` |
| `CloseDown` | 关闭游戏 | — |

示例：
```
maa_start_task('[{"type":"StartUp","params":{"client_type":"Official"}},{"type":"Fight","params":{"stage":"1-7","times":5}}]')
```
